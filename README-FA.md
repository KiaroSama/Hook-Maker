...
| `scripts\` | ویزارد، نصاب، اعتبارسنج و سوییت‌های تست (موتور، هوک‌های گیت‌هاب، RulesCheck، ویزارد، SecretsCheck، AiMemoryLoad/GraphReadCheck، AiMemoryCheck، ContextHooks) |
...
| `Ai-Memory-Load` | قبل از تسک | طبق «Mandatory Startup Gate» سیاست حافظه، `.ai/memory.md` (روتر شروع کار) را مستقیم قبل از شروع تسک وارد context می‌کند — یک فراخوانی Read کمتر لازم است. لیست بقیه‌ی فایل‌های `.ai/*.md` (فقط اسم، نه محتوا) هم اضافه می‌شود تا کل پوشه‌ی `.ai/` دیده شود بدون هزینه‌ی بارگذاری فایل‌هایی که این تسک لازم ندارد. فینگرپرینت بر اساس محتوای memory.md + مجموعه‌ی بقیه‌ی فایل‌ها: تا هیچ‌کدام عوض نشود ساکت می‌ماند. حجم محدود با یادداشت truncate؛ اگر `.ai/memory.md` نباشد ساکت است |
| `Ai-Memory-Check` | بعد از تسک (Stop) | اگر `.ai/memory.md` از آخرین کار قدیمی‌تر باشد، طبق سیاست حافظه درخواست به‌روزرسانی می‌کند — با لیست واقعی فایل‌های تخصصی موجود در `.ai/` (نه یک مثال کلی) تا «هرکدوم صلاحه بروز کن» اسم واقعی داشته باشد — اگر چیز ماندگاری یاد نگرفته، همان‌جا تمام می‌کند |
