...
| `Ci-Status-Check` | بعد از تسک (Stop) | پس از push، تا رسیدن چک‌های **همان کامیت دقیق** به وضعیت نهایی قابل‌قبول اجازه‌ی «تمام» نمی‌دهد. cooldown فقط متن تکراری را کم می‌کند و مجوز پایان را تغییر نمی‌دهد؛ query با SHA دقیق و `--repo` همان upstream انجام می‌شود و نبود دسترسی هرگز به‌عنوان تایید گزارش نمی‌شود |
| `Github-Baseline-Check` | قبل از تسک | فقط وجود YAML را CI حساب نمی‌کند: validation واقعی و blocking، deploy-only/empty، `continue-on-error`، الگوی ناامن PR، اکوسیستم‌های رایج در monorepo عمیق و پوشش `directory/directories` در Dependabot را بررسی می‌کند |
