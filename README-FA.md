...
همه‌ی هوک‌های یادآور برای صرفه‌جویی توکن طراحی شده‌اند: تا سیگنال قطعی نباشد (قدیمی‌بودن، ناسینک‌بودن، وجود wrangler) **ساکت‌اند**، cooldown per-project دارند، هرگز لوپ نمی‌کنند (گارد `stop_hook_active`)، و تصمیم همیشه با خود هوش مصنوعی است — هر یادآور صریحاً اجازه‌ی «هیچ کاری نکن و تمام کن» را هم می‌دهد.
