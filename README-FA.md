# راهنمای Cross-Project Sync Hook (نسخه قابل تنظیم)

این ابزار هیچ مسیر پروژه‌ای را داخل اسکریپت ثابت نکرده است. تمام مسیرها، ارتباط‌ها، رویدادها و قوانین از فایل `sync-hooks.json` خوانده می‌شوند. فایل اصلی فقط یک نمونه غیرفعال و عمومی دارد و هیچ مسیر شخصی در آن ثبت نشده است.
...
| `hooks\` | همه‌ی هوک‌ها — موتور سینک (`CrossProjectSyncHook.ps1`) و هر هوک دلخواهی که بسازی |
...
2. **نصب هوک دلخواه** — از پوشه‌ی `hooks\` یک اسکریپت انتخاب کن، رویدادها را مشخص کن (SessionStart، UserPromptSubmit، لیست دلخواه و ...)، پروژه‌های هدف را بده؛ در همه نصب می‌شود.
