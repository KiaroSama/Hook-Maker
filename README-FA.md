...
| `scripts\` | ویزارد، نصاب، اعتبارسنج و سوییت‌های تست (موتور، هوک‌های گیت‌هاب، RulesCheck، ویزارد، SecretsCheck، AiMemoryLoad/GraphReadCheck) |
...
| `Ai-Memory-Load` | قبل از تسک | طبق «Mandatory Startup Gate» سیاست حافظه، `.ai/memory.md` (روتر شروع کار) را مستقیم قبل از شروع تسک وارد context می‌کند — یک فراخوانی Read کمتر لازم است. بر اساس محتوا فینگرپرینت می‌شود: یک‌بار نشان داده می‌شود، بعد ساکت است تا وقتی memory.md واقعاً عوض شود. حجم محدود با یادداشت truncate؛ اگر `.ai/memory.md` نباشد ساکت است |
