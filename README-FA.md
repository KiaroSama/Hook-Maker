...
| `Ignore-Rules-Check` | قبل تسک (SessionStart) + بعد تسک (Stop) + Git pre-push بومی | الگوهای اجباری فایل‌های محلی/خصوصی را خودکار به `.gitignore` اضافه می‌کند، مسیرهای صریح local-only/never-commit داخل rules پروژه را هم استخراج می‌کند، و تا رفع مشکل هم پایان تسک و هم `git push` را block می‌کند. نصب پروژه‌ای pre-push قبلی را حفظ و زنجیره می‌کند. الگوهای اضافه از `.env` هم قابل تعریف‌اند |
