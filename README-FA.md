...
| `Secrets-Check` | قبل + بعد از تسک (Stop) | `secrets.md` را دقیق و امن نگه می‌دارد: اگر باید ایگنور باشد ولی نیست، یا tracked/staged شده باشد، هشدار می‌دهد؛ اگر یک فایل واقعی `.env*` خودش tracked باشد هشدار می‌دهد؛ اگر مقدار یک سکرت داخل فایل tracked دیگری پیدا شود هشدار می‌دهد (فقط مسیر فایل، هرگز خودِ مقدار چاپ نمی‌شود)؛ سکرت‌هایی که در `.env*` هستند ولی در `secrets.md` نیستند را خودکار اضافه می‌کند (مقدار فقط فایل‌به‌فایل کپی می‌شود، هرگز چاپ/لاگ نمی‌شود)؛ مقادیر شبیه placeholder خالی را هشدار می‌دهد؛ و با فاصله‌ی زیاد (پیش‌فرض هفتگی، نه هر بار) سکرت‌هایی که دیگر جایی در پروژه استفاده نمی‌شوند را با نام (نه مقدار) گزارش می‌کند — **هرگز خودکار حذف نمی‌کند** (یک تشخیص اشتباه می‌تواند یک credential غیرقابل‌بازیابی را نابود کند، پس حذف همیشه با خودِ هوش مصنوعی است، مثل بقیه‌ی هوک‌های یادآور). هیچ تماس شبکه‌ای برقرار نمی‌کند و هرگز سکرت را در برابر سرویس واقعی‌اش تست نمی‌کند |
