...
| `Ignore-Rules-Check` | بعد از تسک (Stop) | الگوهای اجباری فایل‌های محلی/خصوصی را خودکار به `.gitignore` اضافه می‌کند، مسیرهای صریح local-only/never-commit داخل rules پروژه را هم استخراج می‌کند، و اگر مسیر محافظت‌شده هنوز tracked یا staged باشد پایان تسک را block می‌کند تا قبل از push اصلاح شود. الگوهای اضافه از `.env` هم قابل تعریف‌اند |
