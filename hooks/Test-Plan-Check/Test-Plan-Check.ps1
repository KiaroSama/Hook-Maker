...
$excludedDirs = @(Get-HookExcludedDirs -Extra @('vendors', 'coverage', '.cache', 'cache', 'env', '.agents', '.tox', 'site-packages'))
...
[void]$lines.Add('- Cadence: while code is still being written run only LIGHT checks (typecheck, lint, compile/import, one narrowly scoped test, or a smoke run of the changed path). Run the HEAVY pass - full suites, matrices, integration tiers, every guarded run - ONCE after all edits are complete; on failure fix everything that pass reported together, then re-run once. Never re-run a suite because one more file changed.')
[void]$lines.Add('- Optimization before any run: everything that executes code to produce evidence is a test, whatever it is called (suite, pass, matrix, guarded run, benchmark, check). No test runs before it is fully optimized - cheapest tier, one level per behavior, parametrized variants, shared setup, no sleeps, light imports, right CI bucket - new tests written optimized, changed tests re-optimized, EXISTING suites optimized before they run again. An un-optimized run is not permitted: optimize first, then run. Never trade validity for speed. Record suite/date/before-after timing in .ai/TESTING_NOTES.md so an unchanged optimized suite is not re-worked every run.')
[void]$lines.Add('- No orphaned test process: never start a test detached (no &, nohup, Start-Job, Start-Process without -Wait, background tool call) unless the pid and start time are recorded and a bounded owner will reap it; the guarded runner is that owner. Every run has wall + idle ceilings and its whole tree is killed on expiry. Never end your turn or report done while a run you started is alive; an interrupted wait has NO result - kill the tree first. Before finishing, and after every subagent reports, sweep: list what you started (recorded pids by start time, plus test runners/workers/browsers/servers started in this task), terminate every survivor, verify it is gone, report each pid. On Windows check pid AND start time and exclude your own shell.')
[void]$lines.Add('- CI first: every test that can run in GitHub CI runs there, on the pushed commit - the heavy pass always, and any run whose result the next edit does not depend on. Keep local execution to the light loop (typecheck, lint, compile/import, a smoke run of the changed path, at most one narrowly scoped test). Never run locally a test that CI runs: push, wait for the run the push started, read its result for that exact SHA. If the workflow does not cover a suite, extend the workflow - do not run the suite locally instead. Run a test locally in CI''s place only when a live gh query shows CI cannot execute it (billing/quota, no runner, workflow disabled, no remote); record that as a timestamped observation in .ai/TESTING_NOTES.md.')
if ($deepDebug) {
