...
function Test-CiClearedFailure {
    ...
    $no = { param([string]$Why) return [pscustomobject]@{ Cleared = $false; Sha = ''; Reason = $Why } }

    if ($null -eq $FailureEndedUtc) { return (& $no 'the failure records no end time') }
    ...
    if ($null -eq $repo -or [string]::IsNullOrWhiteSpace([string]$repo.Repository)) {
        return (& $no 'this project has no resolvable GitHub remote')
    }

    $head = [string](Invoke-QuietCommand -FilePath git -ArgumentList @('-C', $ProjectRoot, 'rev-parse', 'HEAD'))
    if ($LASTEXITCODE -ne 0 -or [string]::IsNullOrWhiteSpace($head)) { return (& $no 'HEAD cannot be resolved') }
    $head = $head.Trim()
    ...
    if ($LASTEXITCODE -ne 0) { return (& $no 'the working tree state cannot be read') }
    if ($dirty.Count -gt 0) { return (& $no 'the working tree has uncommitted changes, which no CI result describes') }

    $path = Get-CiEvidencePath -ProjectRoot $ProjectRoot -RepoSlug ([string]$repo.Repository)
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { return (& $no 'no CI observation has been recorded for this project yet') }
    $lines = @()
    try { $lines = @([System.IO.File]::ReadAllLines($path)) } catch { return (& $no 'the CI observation cannot be read') }
    ...
    if ($lines.Count -lt 4) { return (& $no 'the CI observation predates this check and says nothing about how it was verified') }
    if ($lines[3].Trim() -cne 'ci-green') { return (& $no 'the recorded CI observation is not an observed all-success result') }
    if ($lines[1].Trim() -cne 'verified') { return (& $no 'the recorded CI outcome is not verified') }
    if ($lines[0].Trim() -cne $head) { return (& $no 'the CI observation is for a different commit than HEAD') }
    ...
    if (-not [DateTime]::TryParse($lines[2].Trim(), $null, [System.Globalization.DateTimeStyles]::RoundtripKind, [ref]$observed)) {
        return (& $no 'the CI observation carries no readable timestamp')
    }
    ...
    if ($observed.ToUniversalTime() -le $failureEnd) { return (& $no 'the CI observation predates the failure it would clear') }
