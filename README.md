...
| `scripts/Test-Wizard.ps1` | Drives the interactive wizard end-to-end via stdin (menu, hook listing, list/range multi-select, Select All = sync group + every hook, client targeting, real self-contained installs, graceful handling of an unwritable project `.ai` directory, hierarchical `<parent>-<slot>` numbering for repeated project-path prompts, installer/idempotency + source-hash verification for the menu-affected hooks) against temp projects (133 assertions). |
...
**last** individual entry. You can also **install a subset** with lists and ranges (e.g. `3-8,19`)
