...
| `scripts/Test-GitHubHooks.ps1` | Offline test suite for the GitHub hooks (101 assertions; mocks git state and `gh`, no network/account). |
| `scripts/Test-RulesCheck.ps1` | Offline test suite for the Rules-Check hook and per-client install targeting (33 assertions). |
| `scripts/Test-Wizard.ps1` | Drives the interactive wizard end-to-end via stdin (menu, hook listing, list/range multi-select, Select All = sync group + every hook, client targeting, real self-contained installs) against temp projects (108 assertions). |
| `scripts/Test-SecretsCheck.ps1` | Offline test suite for the Secrets-Check hook (nested env discovery, critical/advisory pre-push policy, ignore/tracked/staged/index leak, outgoing-commit scan via real bare-remote pushes, throttled unused scan; real throwaway git repos, 58 assertions). |
...
**last** individual entry. You can also **install a subset** with lists and ranges (e.g. `3-8,17`)
