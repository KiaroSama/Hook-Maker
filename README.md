...
| `Ai-Memory-Load` | pre-task (SessionStart, UserPromptSubmit) | Loads `.ai/memory.md` (the startup router) directly into context before work starts, per the AI Context Memory Policy's Mandatory Startup Gate — saves an explicit Read call. Also lists (names only, not content) every other top-level `.ai/*.md` file present, so the agent sees the whole `.ai/` folder without paying to load files the task doesn't need. Fingerprinted on memory.md's content plus the other-file set: shown once, silent until either changes. Capped size with a truncation note; silent when no `.ai/memory.md` exists. |
| `Ai-Memory-Check` | post-task (Stop) | If `.ai/memory.md` is older than the latest work, asks the AI to update the memory per the policy — listing the actual specialized `.ai/*.md` files present so "update whichever deserves it" has real names to weigh — or finish if nothing durable was learned. |
