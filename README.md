...
| `scripts/Test-GitHubHooks.ps1` | Offline test suite for the GitHub hooks (85 assertions; mocks git state and `gh`, no network/account). |
| `scripts/Test-RulesCheck.ps1` | Offline test suite for the Rules-Check hook and per-client install targeting (33 assertions). |
| `scripts/Test-Wizard.ps1` | Drives the interactive wizard end-to-end via stdin (menu, hook listing, list/range multi-select, Select All, client targeting, real self-contained installs) against temp projects (96 assertions). |
| `scripts/Test-SecretsCheck.ps1` | Offline test suite for the Secrets-Check hook (nested env discovery, critical/advisory pre-push policy, ignore/tracked/staged/index leak, throttled unused scan; real throwaway git repos, 44 assertions). |
...
| `Ci-Status-Check` | post-task (Stop) | After a push, blocks "done" until checks for the **exact** pushed commit reach an acceptable terminal state. Pending/failed authorization remains blocking even while repeated detail is cooled down; GitHub queries are throttled, exact-SHA, and bound to the SAME resolved remote used for both repository selection and pushed-state verification (never a mismatched `@{upstream}`). Missing access is never reported as verified. When CI is blocked only by a confirmed **external** condition (GitHub outage, no hosted runner, an org/repo permission failure, an externally-controlled secret being unavailable, a manual approval/environment gate, ...), the agent can record an explicit, evidenced, local-only exception bound to the exact repository + pushed SHA: `-ReportExternalBlocker -Classification <category> -Reason "<evidence>"`. It never marks the commit verified/green, expires after `EXTERNAL_BLOCKER_TTL_MINUTES` (default 1440), and a new commit or different repository invalidates it immediately. |
| `Github-Baseline-Check` | pre-task (SessionStart) | Checks that workflows contain blocking project validation (not empty/deploy-only YAML) — recognizing block-style, flow-style (`on: [push, pull_request]`), bare-value, and block-sequence triggers, `workflow_call` reusable workflows, and multiline `run: \|` validation blocks — flags broad `continue-on-error` and unsafe PR-target execution, detects common ecosystems in pruned deep monorepos (a bare `pyproject.toml` defaults to the `pip` Dependabot ecosystem, which also covers Poetry; reclassified as `uv` only when `uv.lock` is present), and validates `dependabot.yml` directory/directories coverage. Deliberately does not evaluate `if:` step conditions or check least-privilege `permissions:` — a lightweight, deterministic heuristic, not a full YAML/static analyzer. |
...
(not a hook itself) that installs every individual hook below in one pass, using each hook's
recommended events with a shared client/projects answer. The set is derived dynamically from the
hooks actually shipped (never a hard-coded count), so adding or removing a hook folder changes it
automatically. List item `2` is the **sync group**. Individual hooks start at `3`, each showing a
colored `[pre-task]`/`[post-task]` tag and a one-line description, with `Cloudflare-Deploy` fixed
as the **last** individual entry. You can also **install a subset** with lists and ranges (e.g.
`3-8,17`) — use each hook's recommended events with shared client/projects or configure each
separately, then a summary lists exactly what will be installed. Combining `1` with an explicit
pick (e.g. `1,5`) never installs anything twice.

The **sync group** lives inside `1` → **Install an existing hook** as list item `2`
("Create or update a sync group"). Including `2` in a selection (e.g. `2,4-6`) runs the sync-group
