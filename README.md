...
| `hooks/_evidencelib.ps1` | What a CLOSING DECLARATION has to contain before it counts as a claim, dot-sourced by `_hooklib.ps1` as an optional sibling and staged with it. The three closing gates used to test for a PREFIX only, so `MCP used:` with nothing after it cleared the gate, and so did a bare `none` and a line inside a fenced example written to SHOW the format. A prefix is a shape; a claim needs content, and `none` is a real answer only with a reason. |
| `hooks/_taskidentity.ps1` | WHICH USER TASK an event belongs to, dot-sourced by `_hooklib.ps1` as an optional sibling and staged with it. The task is minted ONCE at `UserPromptSubmit` - the only event that means "a new thing was asked for" - into a durable record every later handler reads, so transcript growth, tool output, elapsed time, a retry, compaction or a changed turn id cannot look like a new request. A client that replays a gate's own refusal as the next prompt (Codex does) is recognised by the fingerprint of that refusal and does not mint a task, which is what stopped every refusal refilling the correction allowance. With no record the caller is told the identity is DEGRADED rather than being handed a guess. |
| `hooks/_stoplib.ps1` | The Stop-delivery ledger `_hooklib.ps1` dot-sources as a sibling: the shared correction budget, the per-task chain a gate stands down within, and the reader that returns the CURRENT closing assistant response rather than a raw session tail. Admission is ONE transaction — resolving the chain, rejecting a duplicate claim, checking the remaining allowance and reserving it all happen under a single lock, so contenders cannot each read the same allowance and every one proceed. A chain rotates once per EVENT, and the event is identified by the ORIGINATING USER TASK (`hooks/_taskidentity.ps1`) rather than once per handler, so two gates answering one Stop share it and a duplicate delivery cannot walk past suppression. Identity used to be derived from the transcript's own statistics, which failed in both directions - two handlers of one dispatch could observe different stats while the file was still flushing, and a genuinely new task could observe unchanged ones; that derivation remains only as the bounded degraded path when no task record exists. Admission also separates the semantic FINDING from the delivery, so the same unchanged complaint is not requested twice in a task while evidence that actually changed is evaluated immediately. The ledger publishes atomically and a write that fails REFUSES admission instead of being swallowed — a gate that blocks but cannot record the block would re-arm for ever — and every refusal is kept as an unresolved entry, because spending the allowance ends the corrections rather than making the finding clean. It also carries the FINALIZATION BARRIER: once a task's `DONE / REMAINING` wrap-up has been seen in the closing answer, that is recorded against the task, and every later block in it appends a clause telling the agent this is a correction turn and not to write a second one. Ordering is a separate matter and is stated honestly — on Claude Code the answer is already displayed before Stop runs, so no Stop handler can move a later correction ahead of it; what is enforced is exactly-one wrap-up, plus a pre-task precondition list so the message carrying it is one nothing should interrupt. Loaded optionally, so a runtime installed before it existed keeps the older single-marker fallback instead of failing to start. |
| `hooks/_deliverylib.ps1` | Actor-scoped delivery reservations for informational notices, dot-sourced by `_evidencelib.ps1` and staged with it. A check-then-write delivery stamp let two concurrent handlers each decide they were first and both speak; the key now carries the client, session and actor identity, and validating, reserving and publishing all happen under one stable file lock. A timed reservation expires on its own policy rather than evicting another session's live one to make room, and the store holds hashes and timestamps rather than prompts or command lines. Committing a claim before writing to stdout is at-most-one **attempt**, not exactly-once display: a crash between the two can still lose a visible reminder, and nothing here claims otherwise. |
...
| `scripts/Install-Hook.ps1` | Writes a hook command into a project's `.claude/settings.local.json` (or, when this project's Hook-Maker registrations already live in a tracked `.claude/settings.json`, into THAT file — one project keeps one config rather than a split pair) + `.codex/hooks.json` (or, with no `-TargetProject`, the global `~/.claude` + `~/.codex`). Supports `-CustomHook <path>`. Records the install in `state/install-registry.json` on success (see "Updating previously installed hooks"). |
| `scripts/Validate-Config.ps1` | Validates `sync-hooks.json`. |
| `scripts/Test-Engine.ps1` | Self-contained engine smoke test (73 assertions, runs under pwsh and PowerShell 5.1). |
...
| `scripts/Test-ContextHooks.ps1` | Offline test suite for Mcp-Usage-Check (SessionStart + prompt-relevance-gated UserPromptSubmit) and Skills-Check (active-agent routing — Claude vs Codex, never both policies; global + shared-library + project + `.ai/SKILLS.md` discovery and dedup; same-name/different-content conflict detection; import-guidance and Stop "Skills used:" summary requirement), plus `_hooklib.ps1`'s client-identity resolution — `Get-HookClientId`: an explicit id overrides `CLAUDE_PROJECT_DIR`, an unrecognised explicit id is `unknown` and never Codex, and no signal at all is still Codex — and the guard that its duplicated client-id list matches `_clientcapability.ps1` exactly. The two closing gates are covered here too: the anchored `MCP used:` / `Skills used:` detectors that the hooks' own output can never satisfy, the tool-call evidence each gate requires before it may block, the advisory degradation when a transcript is missing or no tool/skill was used, plugin-skill and shared-library discovery with the exact import command, and the enforcement modes. Skills-Check's per-skill discovery on both clients (install records, the three manifest `skills` shapes, root skills, Desktop and Codex skill folders, disabled and explicit-only skills, partial scans), catalogue drift in both directions, and its fixed routing and intake lines are covered by `_testskilldiscovery.ps1`, from fixtures only. |
...
| `scripts/Test-InstallInvoke.ps1` | Offline test suite for the one installer-invocation and result-validation path (`_installinvoke.ps1`): stub installers returning ok, degraded, failed, trackingFailed, postRegistrationError, malformed, empty and missing results, an unrecognised `overall` value, a throwing installer, a caller-supplied `ResultPath` that cannot override the verdict, and no result document leaked on either the success or the throwing path. Plus the documents that only LOOK like results: a bare `{"overall":"ok"}`, a schema-less one, a future schema, no components, an `ok` contradicted by a failed component, a `failed` with nothing failed, an unrecognised component status, a nameless component, a JSON array and a bare scalar - each `unknown` rather than green or a crash - and a client that was explicitly requested but never mentioned. |
...
| `scripts/_guardedprocess.ps1` | The Windows Job Object, the process tree and the termination decision - everything in the guarded runner that can end a process, split out of `Run-Tests-Guarded.ps1` and kept together on purpose: ownership and killing are one responsibility, and separating them is how a runner ends up terminating something it never owned. `scripts/Run-Tests.ps1` and `scripts/Test-RunTests.ps1` both slice the Job Object C# out of this file as TEXT, so the block and those two readers move together. |
| `scripts/_guardedstate.ps1` | The guarded run's identity, its result document and its live-run marker, split out of `Run-Tests-Guarded.ps1`. Dot-sourced into that script's scope, so `$script:Result` is created here and the run section keeps writing to the same object. |
| `scripts/_guardedtiming.ps1` | The guarded runner's worker budget and the WRITE side of the rolling timing history, split out of `Run-Tests-Guarded.ps1`. The READ side lives in `hooks/_hooklib.ps1`, which this runner deliberately never loads - the sample shape and the `TestTiming-*` path stay compatible across the two by agreement, so changing one means changing the other. |
| `scripts/Run-Tests-Guarded.ps1` | The entry point those three serve: it owns one child process for its whole life, and **refuses to start** (exit 3) if any sibling is missing rather than running a test with no wall ceiling, no idle ceiling and no tree cleanup. The installer ships all four to a hook's runtime or none. |
| `scripts/_testlib.ps1` | Shared helpers the offline PowerShell test suites dot-source: the `Check` assertion recorder, and `Start-BoundedProcess` - the only way a suite should start a child. `Start-Process -Wait` waits for ever, so a hook that blocks on stdin would hang the whole CI bucket to its per-suite ceiling and report a timed-out SUITE instead of naming the child; `Start-BoundedProcess` takes the same parameters (accepting and ignoring `-Wait`/`-PassThru`, so an existing splat needs no edit), waits 180s, then terminates the child and throws naming its pid. |
...
| `scripts/_installregistry.ps1` + `_installregistrystorage.ps1` + `_installregistrymigrate.ps1` | The registry-persistence layer split out of `_installlib.ps1` (which dot-sources the first, which dot-sources the other two, so consumers need no change). `_installregistry.ps1` owns what a RECORD IS — stable identity, per-client subrecords, the record-kind predicates, and the locked atomic upsert. `_installregistrystorage.ps1` owns the bytes on disk — state-directory and registry paths, the one-file-per-record store and its metadata, reading a snapshot back through the directory-listing cache, the single-document legacy form, quarantining a corrupt record or registry, and the crash-aware cross-process lock every write is taken under. `_installregistrymigrate.ps1` owns schema migration — the v1→v2 step that rebuilds per-client subrecords from each client's live registrations, the v2→v3 step that stamps the record kind, and the whole-document pass that applies them on read; every step is idempotent, invents nothing it cannot establish, and never converts a record from one kind into the other. |
| `scripts/_installregistrygeneration.ps1` | Completeness and identity guards for the per-record registry, dot-sourced alongside `_installregistry.ps1`: the generation marker that stops an interrupted write being read as the whole registry (the directory used to become authoritative the moment it existed, so a failure after the first record published a SUBSET while the intact legacy document was ignored), whole-snapshot validation that runs before any byte is written, record id/file-name agreement and id uniqueness, `Test-InstallRegistryShape`, and the post-write check that the bytes on disk really are the record meant to be written - the old check matched the quoted id anywhere in the file, so a record holding it in another field passed, and it now compares a DIGEST of the bytes that were composed, because an older generation of the same record satisfies every id check there is. Completeness is judged the same way: a batch that rewrote two existing records and died after the first leaves both files present, one new and one old, so the marker carries the intended digest of each file and a stale one makes the batch INCOMPLETE. The marker also creates the directory it marks (creating it first left a window in which an empty, unmarked - therefore authoritative - directory existed), the generation-marker name and the Windows device names are refused as record ids, and an interrupted generation is RECOVERED before the next upsert rather than wedging the registry for ever: the records that survived become the registry, nothing is deleted, and a FUTURE schema is refused and never rewritten. |
...
shared `_hooklib.ps1`, `_stoplib.ps1`, `_evidencelib.ps1` and `_taskidentity.ps1`, its `.env` (if present) and — for the sync engine — a copy of the routing
...
`_hooklib.ps1`, `_stoplib.ps1`, `_evidencelib.ps1` and `_taskidentity.ps1`, the hook's own `.env` (hashed as a whole file; its values are never read or stored),
