...
| `hooks/_stoplib.ps1` | The Stop-delivery ledger `_hooklib.ps1` dot-sources as a sibling: the shared correction budget, the per-task chain a gate stands down within, and the reader that returns the CURRENT closing assistant response rather than a raw session tail. Loaded optionally, so a runtime installed before it existed keeps the older single-marker fallback instead of failing to start. |
