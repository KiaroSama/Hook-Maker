...
| `scripts/Test-Wizard.ps1` | Drives the interactive wizard end-to-end via stdin (menu, hook listing, list/range multi-select, Select All = sync group + every hook, client targeting, real self-contained installs, graceful handling of an unwritable project `.ai` directory) against temp projects (114 assertions). |
