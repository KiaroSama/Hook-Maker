...
| `hooks/_timingread.ps1` | The READ side of the bounded rolling test-timing history, dot-sourced by `_hooklib.ps1` as a sibling and staged with it. Split out of that library when it passed the size band that closes a file to new code. The WRITE side lives in the guarded runner (`scripts/_guardedtiming.ps1`) and is deliberately a separate copy, because that runner never loads `_hooklib.ps1` at all: the sample shape and the `TestTiming-*` path stay byte-compatible across the two by agreement, so changing one means changing the other. |
| `hooks/_cbm.ps1` | The Codebase Memory MCP helpers, dot-sourced by `_hooklib.ps1` as a sibling and staged with it. They answer whether a project is indexed and whether that index is current by reading files only - never by executing the server binary - and they own no shared state, which is what made this the cleanest seam when the library was split. |
| `hooks/_transcript.ps1` | The Claude Code transcript reader, dot-sourced by `_hooklib.ps1` as a sibling and staged with it. Kept whole because the JSONL shape it parses - one JSON object per line, where a real newline inside a message arrives as a two-character escape - is the single fact every anchored detector in the shipped hooks depends on. |
