...
# The runtime is materialised from the REAL install plan - now including the two
# artifacts only a Kiro runtime has, the generated kiro-launch.ps1 and this
# install's ownership metadata - so the expected and installed manifests match and
# evaluation actually reaches the registration check.
#
# It used to build the CLIENT-AGNOSTIC plan, which could not describe either file.
# A real Kiro install therefore reported "unexpected managed file:
# <hook>/kiro-launch.ps1" on every evaluation for ever (the launcher is
# regenerated by the very reinstall meant to repair it), and this fixture hid that
# by never creating the launcher at all.
...
try {
    ...
    # Hoisted above the plan: the ownership metadata records this record id, so the
    # id must exist before the runtime is materialised.
    ...
    $kiPlan = Get-InstallPlanFor -HookScript $kiHook -ToolRoot $ToolRoot -FriendlyNameOverride $KiroRegName `
        -IncludeKiroLauncher -RuntimeIdentity (New-RuntimeIdentity -Client 'kiro' -Scope 'project' -RecordId $kiRecordId -ProjectRoot $kiProject)
    ...
    Check 'the kiro fixture runtime carries the planned launcher and ownership metadata' (
        (Test-Path -LiteralPath (Join-Path (Join-Path $kiRuntimeRoot $KiroRegName) 'kiro-launch.ps1') -PathType Leaf) -and
        (Test-Path -LiteralPath (Join-Path (Join-Path $kiRuntimeRoot $KiroRegName) '.hookmaker-runtime.json') -PathType Leaf))
