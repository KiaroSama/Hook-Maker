...
. (Join-Path $PSScriptRoot '_installruntimepayload.ps1')
...
function Get-ManagedInstallPlan {
    param(
        ...
        #
        # This artifact is never opt-in per client: it
    ...
    # The shared libraries every installed runtime carries privately, and the
    # companion executable a single hook needs: both in _installruntimepayload.ps1.
    Add-SharedRuntimeLibraryArtifacts -ToolRoot $ToolRoot -FriendlyName $FriendlyName
    ...
    Add-CompanionRuntimeArtifacts -ToolRoot $ToolRoot -FriendlyName $FriendlyName
    ...
    #
    # runtimeScriptRelativePath is read off the plan rather than assumed from the
    # client id. Deriving it from what the plan ACTUALLY contains makes it
    # impossible for this file to name a runtime script the same plan never
    # produces, however the artifact set grows.
    if ($null -ne $RuntimeIdentity) {
        $runtimeScriptRelative = $FriendlyName + '/' + $FriendlyName + '.ps1'
