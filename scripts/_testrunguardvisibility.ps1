...
# harness, helpers and workspace) - not a standalone suite.
    ...
    $visibleCases = @(
        'Start-Process pwsh -ArgumentList "-File","tests.ps1"',
    ...
    foreach ($silent in @(
            'Start-Process pwsh -Wait -NoNewWindow -ArgumentList "-File","tests.ps1"',
            'Start-Process -FilePath pwsh -WindowStyle Hidden -ArgumentList "-File","tests.ps1"')) {
    ...
    # business, however it is launched.
    $r = Fire -HookPath $hcVis.Script -Cwd $Proj -EventName 'PreToolUse' -Command 'Start-Process notepad.exe' -LocalAppData $hcVis.LocalAppData
    Check 'a non-test command is left alone even when it opens a window' (
